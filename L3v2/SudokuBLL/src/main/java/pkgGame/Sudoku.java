...
public class Sudoku extends LatinSquare {
	...
	public int getRegionNbr(int iCol, int iRow) {
		 ...
		 int b = (iCol / iSqrtSize) + ((iRow / iSqrtSize) * iSqrtSize);
		 return b;
	}
	...
	public void PrintPuzzle() {
		...
		System.out.print(Arrays.toString(super.getRow(i)) + "\n");
	}
	
	
	//i think this works sets the right region with #s 1-9
	// have a test case you should run
	...
	public void SetRegion(int r) {
		...
		for (; i < iMax; i++) {
			for (j = (r % iSqrtSize) * iSqrtSize; j < jMax; j++) {
				LatinSquare[i][j] = iCnt;
				iCnt ++;
			}
			
		}
			super.setLatinSquare(LatinSquare);
		}
	
	private void shuffleArrey (int [] ar) {
		int[] sarr = new int [ar.length];
		Random rand = new Random ();
		for (int i = 0; i < ar.length; i++) { 
		sarr[i] = ar[rand.nextInt()];
	...
	private void shuffleRegion (int r) {
		...
		for	(int j = (r % iSqrtSize) * iSqrtSize; i < ((r % iSqrtSize) * iSqrtSize) + iSqrtSize; j++){
